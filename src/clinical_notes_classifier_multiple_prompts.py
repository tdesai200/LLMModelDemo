...
# ICD-10 Clinical Note Classifier with Multiple Prompt Modes (A/B/C/D)
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="A",
        help="Choose prompt style: A=baseline, B=strong, C=reasoning+JSON, D=disambiguation"
    )
    parser.add_argument(
        "--sample-size",
        type=int,
...
# ---------------------------------------------------------------
# JSON EXTRACTION (supports reasoning text above JSON)
# ---------------------------------------------------------------

def extract_json(text):
    """
    Extracts the first valid JSON object from a string, ignoring any text before it.
    Works with reasoning prompts that include text before JSON (Prompt C).
    """
